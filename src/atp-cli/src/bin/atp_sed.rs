//! atp-sed — POSIX sed-compatible interface with typed structured output.
//!
//! Drop-in replacement for sed that produces ATP's strongly typed
//! JSON/YAML/CSV output while accepting familiar flag syntax:
//!
//! ```bash
//! atp-sed 's/old/new/g' file.txt              # Substitute globally
//! atp-sed -i.bak 's/debug/release/g' *.rs     # In-place with backup
//! atp-sed -e 's/a/b/' -e 's/c/d/' file.txt    # Multiple expressions
//! atp-sed '/pattern/d' file.txt                # Delete matching lines
//! echo "text" | atp-sed 's/text/data/'         # Stdin support
//! ```
//!
//! Output is typed JSON when piped, human-readable when in a terminal.
//! Override with `--format json|yaml|csv|human`.
//!
//! By default, atp-sed operates in DRY-RUN mode (shows what would change).
//! Use `-i` or `--in-place` to modify files.

use anyhow::Result;
use atp_core::output::{format_transform_human, ExecutionMetadata, FileScope};
use atp_core::{
    parse_sed_args, AtpEnvelope, Format, OutputFormatter, Provenance, SedEngine, Walker,
};
use std::io::{self, BufRead, IsTerminal};
use std::time::Instant;

fn main() {
    let args: Vec<String> = std::env::args().skip(1).collect();

    let (format_override, filtered_args) = extract_format_flag(&args);

    let result = run(&filtered_args, format_override);

    if let Err(e) = result {
        eprintln!("atp-sed: {e:#}");
        std::process::exit(2);
    }
}

fn run(args: &[String], format_override: Option<String>) -> Result<()> {
    let start = Instant::now();

    let compat = parse_sed_args(args)?;
    let format = resolve_format(format_override);

    if compat.stdin_mode {
        return run_stdin(&compat, format);
    }

    // File mode — use Walker + SedEngine
    let scope = FileScope {
        roots: compat.files.clone(),
        include_globs: vec![],
        exclude_globs: vec![],
        max_depth: None,
        respect_gitignore: true,
        follow_symlinks: false,
    };
    let walker = Walker::new(scope)?;
    let file_paths = walker.collect_files()?;

    let engine = SedEngine::new(compat.config.clone());
    let path_refs: Vec<&std::path::Path> = file_paths.iter().map(|p| p.as_path()).collect();
    let results = engine.transform_files(&path_refs)?;

    let duration_ms = start.elapsed().as_millis() as u64;

    match format {
        Format::Human => {
            let output = format_transform_human(&results, std::io::stdout().is_terminal());
            print!("{output}");
        }
        _ => {
            let provenance = Provenance::new(
                "sed-compat",
                compat
                    .config
                    .commands
                    .iter()
                    .map(|c| format!("{c:?}"))
                    .collect(),
            );
            let metadata = ExecutionMetadata {
                files_scanned: file_paths.len(),
                files_matched: results.files_modified,
                duration_ms,
                provenance,
            };
            let envelope = AtpEnvelope::new("transform", &results, metadata);
            let output = OutputFormatter::format(&envelope, format)?;
            println!("{output}");
        }
    }

    Ok(())
}

/// Handle stdin mode — read from stdin, apply transforms, output results.
fn run_stdin(compat: &atp_core::SedCompat, format: Format) -> Result<()> {
    let stdin = io::stdin();

    let mut input = String::new();
    for line in stdin.lock().lines() {
        input.push_str(&line?);
        input.push('\n');
    }

    // Write to temp file for engine
    let tmp = std::env::temp_dir().join("atp-sed-stdin.tmp");
    std::fs::write(&tmp, &input)?;

    let engine = SedEngine::new(compat.config.clone());
    let results = engine.transform_files(&[tmp.as_path()])?;

    let _ = std::fs::remove_file(&tmp);

    match format {
        Format::Human => {
            // For human output from stdin, print the transformed text directly
            for file_change in &results.changes {
                for record in &file_change.changes {
                    println!("{}", record.replacement);
                }
            }
        }
        _ => {
            let provenance = Provenance::new(
                "sed-compat",
                compat
                    .config
                    .commands
                    .iter()
                    .map(|c| format!("{c:?}"))
                    .collect(),
            );
            let metadata = ExecutionMetadata {
                files_scanned: 1,
                files_matched: results.files_modified,
                duration_ms: 0,
                provenance,
            };
            let envelope = AtpEnvelope::new("transform", &results, metadata);
            let output = OutputFormatter::format(&envelope, format)?;
            println!("{output}");
        }
    }

    Ok(())
}

fn extract_format_flag(args: &[String]) -> (Option<String>, Vec<String>) {
    let mut format = None;
    let mut filtered = Vec::new();
    let mut skip_next = false;

    for (i, arg) in args.iter().enumerate() {
        if skip_next {
            skip_next = false;
            continue;
        }
        if arg == "--format" {
            if i + 1 < args.len() {
                format = Some(args[i + 1].clone());
                skip_next = true;
            }
        } else if arg.starts_with("--format=") {
            format = Some(arg.trim_start_matches("--format=").to_string());
        } else {
            filtered.push(arg.clone());
        }
    }

    (format, filtered)
}

fn resolve_format(format_override: Option<String>) -> Format {
    if let Some(ref fmt) = format_override {
        fmt.parse().unwrap_or(Format::Json)
    } else if std::io::stdout().is_terminal() {
        Format::Human
    } else {
        Format::Json
    }
}
